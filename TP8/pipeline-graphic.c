...
sem_t barrier_sem; // Sémaphore pour synchroniser les étages
pthread_mutex_t barrier_mutex; // Mutex pour protéger les opérations sur la barrière
...
static void *triangle_stage(void *a)
{
  ...
  {
    puts("production des triangles");
    // Production des triangles dans le tableau des triangles :
    sem_wait(&barrier_sem); // Attendre la barrière
      pthread_mutex_lock(&barrier_mutex); // Verrouiller le mutex pour protéger l'opération
        triangles[triangle_write_index] = triangle;
        triangle_write_index = (triangle_write_index + 1) % NB_TRIANGLES;
      pthread_mutex_unlock(&barrier_mutex); // Déverrouiller le mutex
    sem_post(&barrier_sem); // Libérer la barrière
...
void *raster_stage(void *a)
{
  while (1)
  {
    ...
    puts("consomation des triangles");
