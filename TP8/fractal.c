...
#define NUM_THREADS 12 // Nombre de threads à utiliser
...
void *thread_function(void *arg)
{
    ...
    int pixels_per_thread = size * size / NUM_THREADS;
    int start_pixel = thread_id * pixels_per_thread;
    int end_pixel = (thread_id == NUM_THREADS - 1) ? (size * size) : (start_pixel + pixels_per_thread);

    for (int pixel_index = start_pixel; pixel_index < end_pixel; pixel_index++)
    {
        int y = pixel_index / size;
        int x = pixel_index % size;
